...
public static class ServicePolicies
{
    ...
    /// <summary>
    /// Guards the Ingestor's admin REST surface (TASK-017), which only the Gateway's machine
    /// client — not a browser user — calls. Requires the <c>ingestion:admin</c> scope rather than
    /// a role, matching how the Auth Service's client-credentials grant issues machine tokens.
    /// </summary>
