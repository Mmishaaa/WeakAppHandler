...
{
    /// <summary>
    /// Total flat metric rows the Processor's real normalisation produces from
    /// <see cref="ObservedResponse"/>: one row per motion/energy meter, three per air_quality meter
    /// (co2/pm25/humidity) — 6 + 6 + 6*3 = 30.
    /// </summary>
    public const int ObservedResponseReadingCount = 30;
    ...
    /// batch size the F3 timing scenario exercises is the batch size production sees.
    /// </summary>
